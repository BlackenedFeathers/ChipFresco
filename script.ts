...
type SoundType = 'click' | 'success' | 'message' | 'hover';
...
const SERVICIOS_WINDOWS: ServicioOption[] = [
...
const SERVICIOS_MAC: ServicioOption[] = [
...
const SERVICIOS_CONSOLAS: ServicioOption[] = [
...
const unlockAudio = (): void => {
  ...
  try {
    ...
    ['click', 'touchstart', 'keydown', 'scroll'].forEach(evt =>
      document.body.removeEventListener(evt, unlockAudio, { capture: true } as EventListenerOptions)
    );
  } catch (e) {
    /* noop */
  }
...
document.getElementById('soundToggle')!.addEventListener('click', function (this: HTMLElement) {
...
function openTab(tabId: string): void {
  ...
  const elements = document.getElementById('tab-' + tabId)!.querySelectorAll<HTMLElement>('.reveal');
  elements.forEach(el => {
...
/* ================= SLIDERS ================= */
function initSlider(sliderId: string): void {
...
function promptService(serviceName: string): void {
...
const inputEl = document.getElementById('botInput') as HTMLInputElement;
let botState: BotState = 'init';
let userData: UserData = { nombre: '', equipo: '', servicio: '', puntoEncuentro: '', horario: '' };

function scrollToBottom(): void {
  msgsEl.scrollTop = msgsEl.scrollHeight;
}
function addBotMsg(text: string): void {
...
function botFlow(preSelectedService?: string): void {
  ...
  if (preSelectedService) userData.servicio = preSelectedService;
...
function selectEquipo(equipoStr: string): void {
  ...
  let servicios: ServicioOption[];
  if (equipoStr === 'Mac / MacBook') servicios = SERVICIOS_MAC;
  else if (equipoStr === 'Consola') servicios = SERVICIOS_CONSOLAS;
  else servicios = SERVICIOS_WINDOWS;
...
function selectServicio(servicioLabel: string): void {
...
function finalizarFlujoArmado(): void {
  clearChips();
  inputEl.disabled = true;
  ...
  setTimeout(() => {
    ...
    const resumen =
      '¡Hola Técnico! Quiero armar una PC Custom 🖥️\n\n👤 Nombre: ' + userData.nombre + '\n📝 Detalles: ' + userData.servicio;
...
function explicarHorarios(): void {
...
function finalizarFlujo(punto: string): void {
  addUserMsg(punto);
  ...
  setTimeout(() => {
    ...
    const resumen =
      '¡Hola Técnico! Quiero confirmar una cita 🛠️\n\n👤 Nombre: ' +
      userData.nombre +
      '\n🔧 Servicio: ' +
      userData.servicio +
      '\n⏰ Día/Hora: ' +
